...
if [  $# -eq 0 ]; then
    ...
    [[ -f $dirs[$i]/$files[$i] ]] && cp -v $dirs[$i]/$files[$i] $archdir/$files[$i] && ((k=k+1))
    done
    echo "Config Files backed up : $k"
elif [ $1 = "restore" ] ; then
    for ((i=1;i<=${#files};i++)) do
        [[ -d  $dirs[$i] && -f $archdir/$files[$i] ]] && cp -v $archdir/$files[$i] $dirs[$i]/$files[$i] && ((k=k+1))
    done
    ...
    for ((i=1;i<=${#files};i++)) do
        [ -d $dirs[$i] ] && cp -v $archdir/$files[$i] $dirs[$i]/$files[$i] && ((k=k+1))
    done
